    ...
    <div class="page-header">
      <div class="page-header__left">
        ...
        <div>
          ...
          <p class="page-subtitle">Upload & configure a new track to the library</p>
        </div>
      </div>
    ...
    <div class="steps-bar">
      <div v-for="(step, i) in steps" :key="i" class="step"
        ...
        <div class="step-circle">
          <svg v-if="currentStep > i" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor"
            stroke-width="3">
            <polyline points="20 6 9 17 4 12" />
          </svg>
      ...
      <div class="col-main">
        ...
        <section v-show="currentStep === 0" class="form-section">
          ...
          <div class="form-grid form-grid--2">
            ...
            <div class="field">
              ...
              <input v-model="form.license_type" type="text" class="field-input"
        ...
        <section v-show="currentStep === 1" class="form-section">
          <div class="section-heading">
            ...
            <h2 class="section-title">Audio File <span class="required" style="font-size:13px">*</span></h2>
          </div>
          ...
          <div class="audio-info-banner">
            ...
            <span>Upload a single original file. The backend will automatically handle splitting it into parts.
              <strong>Standard / HQ / Lossless</strong> Then upload it to Cloudinary.</span>
          </div>

          <!-- Upload zone -->
          <div class="upload-zone"
            :class="{ dragover: isDragging, 'has-file': audioFile, 'upload-zone--error': stepErrors[1].audio_file }"
            ...
            <template v-if="!audioFile">
              <div class="upload-icon" :class="{ 'upload-icon--error': stepErrors[1].audio_file }">
              ...
              <p class="upload-title">Drag & drop audio file here</p>
            ...
            <template v-else>
              <div class="upload-preview">
                ...
                <div class="upload-preview__info">
                  ...
                  <p class="upload-preview__size">{{ formatBytes(audioFile.size) }} · {{ formatDuration(form.duration)
                    }}</p>
                </div>
          ...
          <!-- Validation error for upload zone -->
          <p v-if="stepErrors[1].audio_file" class="field-error" style="margin-top:-8px">
            {{ stepErrors[1].audio_file }}
          </p>
            ...
            <div class="pipeline-steps">
              <div class="pipeline-step" :class="{ 'pipeline-step--active': !!audioFile }">
                ...
                <div>
                  ...
                  <p class="pipeline-step__sub">{{ audioFile ? audioFile.name : 'Chưa có file' }}</p>
                </div>
              </div>
      ...
      <div class="col-preview">
        ...
        <div class="payload-preview">
          <div class="payload-header">
            <p class="payload-title">API Payload</p>
...
function validateStep(step: number): boolean {
  ...
  if (step === 1) {
    const e: Step1Errors = {}
    // Kiểm tra audioFile ref — đây là nguồn truth duy nhất cho UI
    if (!audioFile.value) e.audio_file = 'Vui lòng chọn file audio để tiếp tục.'
    stepErrors.value[1] = e
    return Object.keys(e).length === 0
  }
...
// ── Form ──
const form = reactive<CreateSongPayload>({
...
// form.audio_file luôn được sync với audioFile
...
const { uploadCover, isUploading, uploadProgress } = useCloudinaryUpload()

// ── Watch audioFile → sync form.audio_file + object URL ──
watch(audioFile, (newFile) => {
  // Luôn sync form để payload gửi lên backend đúng
  form.audio_file = newFile
  ...
  if (newFile) {
    audioObjectUrl.value = URL.createObjectURL(newFile)
  }
})
...
const payloadJson = computed<string>(() => {
  ...
  // Hiển thị tên file thay vì object File trong JSON preview
...
function removeAudio(): void {
  ...
  currentTime.value = 0
  stopWave()
  audioFile.value = null  // watch tự revoke URL + sync form.audio_file = null
  form.duration   = 0
  form.file_size  = 0
}
...
function saveDraft(): void {
  form.status = 'draft'
  alert('Saved as draft! (Connect to Laravel API)')
}
...
.wave-bar {
  flex: 1;
  background: linear-gradient(180deg, #00c6ff, rgba(0, 114, 255, .3));
  border-radius: 2px;
  min-height: 4px;
  opacity: .7;
}
...
.audio-info-banner svg {
  flex-shrink: 0;
  margin-top: 1px;
  color: #00c6ff;
}

.audio-info-banner strong {
  color: #00c6ff;
}
...
.pipeline-step__dot {
  ...
  border-radius: 50%;
  background: #43e97b;
  box-shadow: 0 0 6px rgba(67, 233, 123, .5);
  flex-shrink: 0;
}
...
@keyframes pulse {
  ...
  50% {
    box-shadow: 0 0 14px rgba(67, 233, 123, .9);
  }
}
...
.yt-progress__fill {
  ...
  top: 0;
  height: 100%;
  background: #00bef5; /* YouTube red — đổi thành #00c6ff nếu muốn màu app */
  border-radius: 0;
  pointer-events: none;
}
...
.yt-progress:hover .yt-progress__thumb {
