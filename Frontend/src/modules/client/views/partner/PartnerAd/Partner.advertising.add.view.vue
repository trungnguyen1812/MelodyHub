  ...
  <div class="wizard-shell">
    ...
    <div class="topbar">
      ...
      <div class="topbar-title">
        <span class="topbar-label">Create Campaign</span>
      </div>
      ...
      <transition name="slide-fade" mode="out-in">
        <div :key="currentStep" class="step-panel">
          ...
          <div v-if="currentStep === 4" class="fields-col">
            ...
            <!-- Priority display (read-only, reflects tier chosen in Budget step) -->
          ...
          <div v-if="currentStep === 6" class="fields-col">
            ...
            <div v-if="walletDanger" class="submit-blocker">
              ...
              Wallet balance is insufficient. Please top up before launching.
            </div>
          </div>
...
console.log('Wizard Genres:', genres.value);

// ── Wallet balance ─────────────────────────────────────
const walletBalance = computed(() => {
  return Number(partnerStore.user?.wallet_balance || 0)
})
...
onMounted(async () => {
  ...
  if (partnerStore.partnerInfo?.partner) {
    const p = partnerStore.partnerInfo.partner
    form.partner_id = p.id
    if (!form.advertiser_name) {
...
const cpmWarning = computed(() => {
  ...
  return form.cost_per_play < currentTier.value.minCpm
})

const cpcWarning = computed(() => {
  if (!form.cost_per_click) return false
  return form.cost_per_click < currentTier.value.minCpc
})

const walletWarning = computed(() => {
  if (!form.budget_total) return false
  return form.budget_total > walletBalance.value * 0.8 && form.budget_total <= walletBalance.value
})

const walletDanger = computed(() => {
  if (!form.budget_total) return false
  return form.budget_total > walletBalance.value
})
...
const dailySpend = computed(() => {
  if (!campaignDays.value || !form.budget_total) return '$0'
  return '$' + (form.budget_total / campaignDays.value).toFixed(2)
})
...
function enforceCpmFloor() {
  if (form.cost_per_play && form.cost_per_play < currentTier.value.minCpm) {
    form.cost_per_play = currentTier.value.minCpm
  }
}

function enforceCpcFloor() {
  if (form.cost_per_click && form.cost_per_click < currentTier.value.minCpc) {
    form.cost_per_click = currentTier.value.minCpc
  }
}
...
async function handleSubmit() {
  ...
  try {
    ...
    const msg = error.response?.data?.message || 'Failed to launch campaign'
    showToast(msg, 'error')
