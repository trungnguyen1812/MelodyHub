<template>
    <div class="dashboard-container">
        <div class="header-section">
            <div class="title-container">
                ...
                <p class="subtitle">Manage All Genres Accounts</p>
            </div>
            <div class="header-actions">
                <button @click="CreateGenre" class="btn-add-user">
                    <span class="btn-icon">
                ...
                <div class="search-box">
                    <input type="text" placeholder="Search genres..." v-model="keyword" @input="onSearch">
        ...
        <div class="table-section">
                        ...
                        <tr v-for="genre in paginatedGenres" :key="genre.id">
                            <td class="user-cell">
                                <div class="user-avatar">
                                    <img :src="getFullImageUrl(genre.avatar_url)" :alt="genre.name"
                                        class="avatar-img" />
                                </div>
                                <div class="user-info">
                                    <p class="user-name" :title="genre.name">{{ genre.name }}</p>
                                </div>
                            </td>
                            <td>{{ formatCompactNumber(genre.total_followers) }}</td>
                            <td>{{ formatCompactNumber(genre.total_songs) }}</td>
                            <td>
                                <span>
                                    {{ genre.country }}
                                </span>
                            ...
                            <td>
                                <span :class="`status-badge status-${genre.status}`">
                                    {{ genre.status }}
                                </span>
                            </td>
                            <td>{{ formatDate(genre.created_at ?? "") }}</td>
                            <td>
                                <div class="social-icons">
                                    <a v-if="genre.facebook_url" :href="genre.facebook_url" target="_blank" class="social-icon facebook">
                                    ...
                                    <a v-if="genre.instagram_url" :href="genre.instagram_url" target="_blank" class="social-icon instagram">
                                        <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
                                    ...
                                    <a v-if="genre.twitter_url" :href="genre.twitter_url" target="_blank" class="social-icon x">
                                        <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
                                            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                                        </svg>
                                    </a>
                                    <a v-if="genre.youtube_url" :href="genre.youtube_url" target="_blank" class="social-icon youtube">
                                        <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
                            ...
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-action btn-edit"  @click="viewUpdateGenre(genre.id)">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                                            <path d="m5.433 13.917 1.262-3.155A4 4 0 0 1 7.58 9.42l6.92-6.918a2.121 2.121 0 0 1 3 3l-6.92 6.918c-.383.383-.84.685-1.343.886l-3.154 1.262a.5.5 0 0 1-.65-.65Z" />
                                            <path d="M3.5 5.75c0-.69.56-1.25 1.25-1.25H10A.75.75 0 0 0 10 3H4.75A2.75 2.75 0 0 0 2 5.75v9.5A2.75 2.75 0 0 0 4.75 18h9.5A2.75 2.75 0 0 0 17 15.25V10a.75.75 0 0 0-1.5 0v5.25c0 .69-.56 1.25-1.25 1.25h-9.5c-.69 0-1.25-.56-1.25-1.25v-9.5Z" />
                                        </svg>
                                    </button>
                                    <button class="btn-action btn-delete" @click="deleteUser(genre.id)">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                                            <path fill-rule="evenodd" d="M8.75 1A2.75 2.75 0 0 0 6 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 1 0 .23 1.482l.149-.022.841 10.518A2.75 2.75 0 0 0 7.596 19h4.807a2.75 2.75 0 0 0 2.742-2.53l.841-10.52.149.023a.75.75 0 0 0 .23-1.482A41.03 41.03 0 0 0 14 4.193V3.75A2.75 2.75 0 0 0 11.25 1h-2.5ZM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4ZM8.58 7.72a.75.75 0 0 0-1.5.06l.3 7.5a.75.75 0 1 0 1.5-.06l-.3-7.5Zm4.34.06a.75.75 0 1 0-1.5-.06l-.3 7.5a.75.75 0 1 0 1.5.06l.3-7.5Z" clip-rule="evenodd" />
                                        </svg>
                                    </button>
                                    <button class="btn-action btn-view" @click="viewDetailGenre(genre.id)">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
            ...
            <!-- Pagination -->
            <div v-if="genres.length > 0" class="pagination">
                <div class="pagination-info">
                    Showing {{ paginationStart }} to {{ paginationEnd }} of {{ genres.length }} entries
                </div>
    ...
    <div v-else-if="genres.length === 0" class="empty-state">
...
const CreateGenre =()=>{
    router.push({name:"admin.genresmanager.add"});
}
...
const onSearch = ()=>{
    ...
    searchTimeout = window.setTimeout(async() => {
        if (!keyword.value.trim()) {
         ...
         await genreStore.fetchSearchArtitst(
            keyword.value
        );
    }, 300);
}
...
// FOR ARTISTS
function viewDetailGenre(id: number) {
    router.push({
...
function viewUpdateGenre( id: number) {
    router.push({
...
async function deleteUser(id: number) {
    try {
        ...
        await genreStore.fetchGenres();
        notificationStore.notify("Delete genre successful", "success");

        router.push({name:"admin.genresmanager.all"});

    } catch (error: any) {
        const err = error as { response?: { status?: number } }
        notificationStore.notify("Delete genre error", "error");
...
const totalPages = computed(() => Math.ceil(genres.value.length / itemsPerPage));
const paginationStart = computed(() => ((currentPage.value - 1) * itemsPerPage) + 1);
const paginationEnd = computed(() => Math.min(currentPage.value * itemsPerPage, genres.value.length));
