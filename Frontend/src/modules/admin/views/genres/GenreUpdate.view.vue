  ...
  <div class="genre-shell">
    ...
    <header class="topbar">
      ...
      <div class="topbar-center">
        <span class="topbar-label">Edit Genre: {{ form.name }}</span>
      </div>
            ...
            <div class="genre-card-preview" :style="{ background: form.color ? form.color + '22' : 'rgba(255,255,255,0.04)', borderColor: form.color ? form.color + '55' : 'rgba(255,255,255,0.08)' }">
              ...
              <div class="gcp-body">
                ...
                <div class="gcp-meta">
                  ...
                  <span class="gcp-songs">{{ form.total_songs || 0 }} songs</span>
                </div>
              </div>
            </div>
...
const parentGenres = computed(() =>
  (genres.value || []).filter((g: any) => !g.parent_id && g.id !== Number(route.params.id))
)
...
// Don't watch name for slug in update mode to prevent accidental slug changes
// Unless the user explicitly wants to generate it
...
const loadGenreData = async () => {
  try {
    ...
    Object.assign(form, {
      ...
      order: data.order || 0,
      is_active: data.status === 'active' || data.is_active === true || data.is_active === 1,
      total_songs: data.total_songs || 0
    })

    if (data.cover_url) {
      coverPreview.value = getFullImageUrl(data.cover_url)
    }

  } catch (err: any) {
    console.error('Error loading genre:', err)
    notificationStore.notify('Failed to load genre details', 'error')
...
const submitForm = async () => {
  ...
  try {
    ...
    // The backend might expect 'bio' and 'status' instead of 'description' and 'is_active'
    // based on the payload interface I saw earlier
    payload.bio = form.description
    payload.status = form.is_active ? 'active' : 'inactive'
    
    if (coverFile.value) {
       payload.cover_url = coverFile.value
    }

    await genreStore.fetchUpdate(Number(route.params.id), payload)
    notificationStore.notify('Genre updated successfully!', 'success')
...
.topbar-center { flex: 1; margin-left: 20px; }
...
.fs-val { font-size: 12px; color: #fff; font-weight: 500; }
