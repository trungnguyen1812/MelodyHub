      ...
      <div class="page-header__left">
        ...
        <div class="page-title-group">
          ...
          <p class="page-subtitle">
            ...
            <span v-if="songStore.loading" class="loading-dots">Loading<span>.</span><span>.</span><span>.</span></span>
            <span v-else><strong>{{ songStore.meta?.total ?? 0 }}</strong> tracks</span>
          </p>
        </div>
      </div>
...
.ambient-orb { position: absolute; border-radius: 50%; filter: blur(120px); opacity: 0.04; }
.ambient-orb--1 { width: 600px; height: 600px; top: -200px; left: -100px; background: var(--accent); }
.ambient-orb--2 { width: 500px; height: 500px; bottom: -200px; right: -100px; background: #8b5cf6; }

.page-header, .toolbar, .loading-wrap,
.error-state, .table-section, .grid-view,
.empty-state, .pagination { position: relative; z-index: 1; }
...
.btn-back {
  ...
  padding: 8px 14px; background: var(--surface); border: 1px solid var(--border-2);
  border-radius: 10px; color: var(--text-2); font-size: 13px; font-weight: 500;
  cursor: pointer; transition: all .2s; flex-shrink: 0;
}
.btn-back:hover { border-color: var(--accent); color: var(--accent-2); background: rgba(59,130,246,.06); }
...
.page-subtitle strong { color: var(--text-2); }

.btn-add {
  display: flex; align-items: center; gap: 8px;
  padding: 10px 20px; background: linear-gradient(90deg, #00c6ff, #0072ff);
