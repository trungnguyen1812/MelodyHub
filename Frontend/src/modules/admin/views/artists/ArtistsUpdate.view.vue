...
const loadArtistData = async () => {
    try {
        ...
        if (res && res.data) {
            ...
            // Port images
            (form as any).avatar_url = data.avatar_url || data.avatar;
            (form as any).banner_url = data.banner_url || data.banner;
        }
    } catch (err: any) {
        notificationStore.notify("Failed to load artist data", "error");
...
const displayAvatar = computed(() => {
    ...
    if ((form as any).avatar_url) return getFullImageUrl((form as any).avatar_url);
    return defaultAvatar;
});

const displayBanner = computed(() => {
    if (bannerFile.value) return URL.createObjectURL(bannerFile.value);
    if ((form as any).banner_url) return getFullImageUrl((form as any).banner_url);
...
const handleSubmit = async () => {
    try {
        ...
        Object.keys(payload).forEach(key => {
            if (payload[key] === '' && key !== 'avatar' && key !== 'banner') {
                payload[key] = null;
            }
        });

        await artistStore.fetchUpdate(id, payload);
        notificationStore.notify("Artist updated successfully!", "success");
        router.push({ name: "admin.artistsmanager.all" });
    } catch (err: any) {
        notificationStore.notify(err.response?.data?.message || "Failed to update artist", "error");
    } finally {
