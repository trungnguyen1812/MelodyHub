    ...
    <div class="dashboard-container">
        <div class="header-section">
            <div class="title-container">
                ...
                <p class="subtitle">Manage All Albums Accounts</p>
            </div>
            <div class="header-actions">
                <button @click="CreateAlbum" class="btn-add-user">
                    <span class="btn-icon">
                ...
                <div class="search-box">
                    <input type="text" placeholder="Search albums..." v-model="keyword" @input="onSearch">
        ...
        <div class="table-section">
                        ...
                        <tr v-for="album in paginatedAlbums" :key="album.id">
                            <td class="user-cell">
                                <div class="user-avatar">
                                    <img :src="getFullImageUrl(album.cover_url)" :alt="album.name"
                                        class="avatar-img" />
                                </div>
                                <div class="user-info">
                                    <p class="user-name" :title="album.name">{{ album.name }}</p>
                                    <p class="user-meta">{{ album.slug }}</p>
                                </div>
                            </td>
                            <!-- <td class="artist-cell">{{ album.artist_id??0 }}</td> -->
                            <td>
                                <span class="album-type-badge" :class="`type-${album.album_type}`">
                                    {{ album.album_type }}
                                </span>
                            </td>
                            ...
                            <td class="text-center">
                                <span v-if="album.is_featured" class="badge-featured">⭐</span>
                                <span v-else class="badge-empty">—</span>
                            </td>
                            <td class="text-center">
                                <span v-if="album.is_premium" class="badge-premium">💎</span>
                                <span v-else class="badge-empty">—</span>
                            </td>
                            <td>
                                <div class="action-buttons">
                                    <button class="btn-action btn-edit"  @click="viewUpdateAlbum(album.id)" title="Edit">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                                            <path d="m5.433 13.917 1.262-3.155A4 4 0 0 1 7.58 9.42l6.92-6.918a2.121 2.121 0 0 1 3 3l-6.92 6.918c-.383.383-.84.685-1.343.886l-3.154 1.262a.5.5 0 0 1-.65-.65Z" />
                                            <path d="M3.5 5.75c0-.69.56-1.25 1.25-1.25H10A.75.75 0 0 0 10 3H4.75A2.75 2.75 0 0 0 2 5.75v9.5A2.75 2.75 0 0 0 4.75 18h9.5A2.75 2.75 0 0 0 17 15.25V10a.75.75 0 0 0-1.5 0v5.25c0 .69-.56 1.25-1.25 1.25h-9.5c-.69 0-1.25-.56-1.25-1.25v-9.5Z" />
                                        </svg>
                                    </button>
                                    <button class="btn-action btn-delete" @click="deleteAlbum(album.id)" title="Delete">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
                                            <path fill-rule="evenodd" d="M8.75 1A2.75 2.75 0 0 0 6 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 1 0 .23 1.482l.149-.022.841 10.518A2.75 2.75 0 0 0 7.596 19h4.807a2.75 2.75 0 0 0 2.742-2.53l.841-10.52.149.023a.75.75 0 0 0 .23-1.482A41.03 41.03 0 0 0 14 4.193V3.75A2.75 2.75 0 0 0 11.25 1h-2.5ZM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4ZM8.58 7.72a.75.75 0 0 0-1.5.06l.3 7.5a.75.75 0 1 0 1.5-.06l-.3-7.5Zm4.34.06a.75.75 0 1 0-1.5-.06l-.3 7.5a.75.75 0 1 0 1.5.06l.3-7.5Z" clip-rule="evenodd" />
                                        </svg>
                                    </button>
                                    <button class="btn-action btn-view" @click="viewDetailAlbum(album.id)" title="View">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="size-5">
            ...
            <!-- Pagination -->
            <div v-if="albums.length > 0" class="pagination">
                <div class="pagination-info">
                    Showing {{ paginationStart }} to {{ paginationEnd }} of {{ albums.length }} entries
                </div>
...
const CreateAlbum =()=>{
    router.push({name:"admin.albumsmanager.add"});
}
...
const onSearch = ()=>{
    ...
    searchTimeout = window.setTimeout(async() => {
        if (!keyword.value.trim()) {
         ...
         await albumStore.fetchSearchArtitst(
            keyword.value
        );
    }, 300);
}
...
// FOR ARTISTS
function viewDetailAlbum(id: number) {
    router.push({
...
function viewUpdateAlbum( id: number) {
    router.push({
...
async function deleteAlbum(id: number) {
    try {
        ...
        await albumStore.fetchAlbums();
        notificationStore.notify("Delete album successful", "success");

        router.push({name:"admin.albumsmanager.all"});

    } catch (error: any) {
        const err = error as { response?: { status?: number } }
        notificationStore.notify("Delete album error", "error");
...
const paginationEnd = computed(() => Math.min(currentPage.value * itemsPerPage, albums.value.length));
...
.album-type-badge.type-album {
    background: rgba(0, 170, 255, 0.2);
    color: #66ccff;
    border: 1px solid rgba(0, 170, 255, 0.3);
}

.album-type-badge.type-single {
    background: rgba(255, 192, 61, 0.2);
    color: #ffda6f;
    border: 1px solid rgba(255, 192, 61, 0.3);
}

.album-type-badge.type-ep {
    background: rgba(255, 87, 34, 0.2);
    color: #ffb380;
    border: 1px solid rgba(255, 87, 34, 0.3);
}

.album-type-badge.type-compilation {
    background: rgba(156, 39, 176, 0.2);
    color: #e68dff;
    border: 1px solid rgba(156, 39, 176, 0.3);
}

.album-type-badge.type-live {
