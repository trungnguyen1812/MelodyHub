...
const loadAlbumData = async () => {
  try {
    ...
    if (res) {
      ...
      // Slice out the time portion if release_date has it (e.g. "2024-01-01 00:00:00")
...
const submitForm = async () => {
  try {
    ...
    // If cover_url isn't a new File, we shouldn't send the string URL back,
    // so we delete it from the payload to avoid resetting or failing validation.
    if (!(payload.cover_url instanceof File)) {
      delete payload.cover_url;
    }
    
    await albumStore.fetchUpdate(albumId, payload)
