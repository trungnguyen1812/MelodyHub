          ...
          <tbody>
            <template v-if="!loading">
              <tr v-for="album in paginatedAlbums" :key="album.id" class="g-row">
                ...
                <td>
                  <div class="actions">
                    <button class="act-btn edit" @click="viewUpdateAlbum(album.id)" title="Edit">
                    ...
                    <button class="act-btn view" @click="viewDetailAlbum(album.id)" title="View">
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" width="14" height="14">
...
const filteredAlbums = computed(() => {
  if (!keyword.value.trim()) return albums.value
  const k = keyword.value.toLowerCase()
  return albums.value.filter((a: any) => 
    a.name?.toLowerCase().includes(k) || 
    a.label?.toLowerCase().includes(k) ||
    a.artist?.name?.toLowerCase().includes(k)
  )
})

const totalPages = computed(() => Math.max(1, Math.ceil(filteredAlbums.value.length / itemsPerPage)))
...
const paginatedAlbums = computed(() => {
  ...
  return filteredAlbums.value.slice(s, s + itemsPerPage)
})
...
const CreateAlbum = () => router.push({ name: 'admin.albumsmanager.add' })
const viewDetailAlbum = (id: number) => router.push({ name: 'admin.albumsmanager.detail', params: { id } })
const viewUpdateAlbum = (id: number) => router.push({ name: 'admin.albumsmanager.update', params: { id } })
