    ...
    public function update(Request $request, Song $song): JsonResponse
    {
        ...
        try {
            ...
            $fields = [
                ...
                'duration', 'bitrate', 'quality', 'partner_id', 'status',
            ];
            if ($request->has('lyrics')) {
                $lyrics = $request->lyrics;
                
                if (is_array($lyrics)) {
                    $updateData['lyrics'] = json_encode($lyrics, JSON_UNESCAPED_UNICODE);
                } elseif (is_string($lyrics) && $lyrics !== '[object Object]' && !empty($lyrics)) {
